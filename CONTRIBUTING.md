...
13. **Ambient monitors are vein definitions, and the built-ins are the reference definitions — read them before registering a new block.** A new ambient capability is a definition JSON (`veins/`) composed of generic blocks wherever the logic is generic; only a genuinely bespoke source registers a code-backed block (`vein_engine.register`, from the module that owns the domain, `monitor=True` when its items are standing situations that retire on clear). Card posting, dedup, quiet discipline, and scheduling belong to the engine, never to a block.
