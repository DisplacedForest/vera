...
nodes/edges/threads into the Profile Graph through the profile graph's deterministic dedup/decay math.
...
async def merge_conversation(conv, extracted, now=None):
    """Land one conversation's extracted nodes/edges/threads in the graph through the profile graph's
