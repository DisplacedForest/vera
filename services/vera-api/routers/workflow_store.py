...
def _pulse_definition() -> dict:
    return {
        ...
        "nodes": [
            ...
            {"id": "cover_art", "type": "pulse.cover_art", "label": "Cover art", "icon": "photo", "tint": "purple"},
...
def init():
    with _conn() as conn:
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS workflow_node_runs (
            ...
            state TEXT NOT NULL, output TEXT, created_at INTEGER NOT NULL)""")
...
def record_node_runs(run_id: str, output: dict):
    ...
    with _conn() as conn:
        ...
        conn.executemany("INSERT INTO workflow_node_runs(id, workflow_run_id, node_id, state, output, created_at) VALUES(?,?,?,?,?,?)",
                         [(str(uuid.uuid4()), run_id, node_id, status, json.dumps(node_output), now) for node_id, node_output in rows])
...
def latest_run(workflow_id: str) -> dict | None:
    ...
    with _conn() as conn:
        ...
        if not row:
            return None
        nodes = conn.execute("SELECT node_id, state, output, created_at FROM workflow_node_runs WHERE workflow_run_id=? ORDER BY created_at", (row["id"],)).fetchall()
    return {"id": row["id"], "workflow_id": row["workflow_id"], "workflow_version_id": row["workflow_version_id"],
            ...
            "nodes": [{"id": node["node_id"], "state": node["state"], "output": json.loads(node["output"] or "{}"), "created_at": node["created_at"]} for node in nodes]}
