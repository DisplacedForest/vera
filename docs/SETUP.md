...
| Web search | `SEARXNG_BASE` (+ optional `PLAYWRIGHT_WS`) | Research, Pulse sourcing, watcher veins |
...
Opening the Pulse flow switches the existing app sidebar to the approved node library. Drag nodes into the main workflow canvas, select a node to configure it in the right inspector, and use an explicit draft promotion when the graph is ready. Editing a draft does not change the published Pulse workflow.
