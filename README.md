...
Native chat uses the standard OpenAI Chat Completions `tools` and streamed `tool_calls` fields. It validates names and JSON arguments before local execution, returns ordinary `tool` messages to the model, and continues until the model sends its final answer. The loop stops after four tool rounds or eight calls, retaining received text and activity when a tool or endpoint fails. Apple Reminders is the first built-in. It is disabled by default, requests macOS permission when deliberately enabled, and supports explicit chat requests to list, create, and complete reminders. Each call appears as a live expandable chip and remains with the conversation after relaunch. Endpoints without standard tool calling continue as text chat and receive no private text protocol. Memory retrieval, documents, attachments, voice, additional native tools, MCP, and Open WebUI history import remain separate work.
