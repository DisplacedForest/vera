...
Pipeline veins register their jobs dynamically — one per definition, appearing and disappearing with the definition file — and the standard override convention applies (`SCHEDULE_VEIN_<KIND>`, `SCHEDULE_VEIN_<KIND>_ENABLED`). The built-in definitions ship with System every 15 minutes (service health probes plus stack-update checks; the LLM floor makes the effective compose cadence 30 minutes), Weather every 6 hours, and Media on Sunday mornings (additionally gated on the Overseerr media-curation consent).
